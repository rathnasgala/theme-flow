...
test('ships the immutable Flow release contract', async () => {
  const { manifest, cssBytes } = await verifyTheme();
  const packageJson = JSON.parse(await readFile(path.join(ROOT, 'package.json'), 'utf8'));

  assert.equal(manifest.id, 'flow');
  ...
  assert.equal(packageJson.name, '@rathnasgala/theme-flow');
...
async function withFixture(assertions) {
  const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'gala-flow-'));
  const fixture = path.join(temporaryDirectory, 'theme-flow');
  try {
    await mkdir(fixture);
